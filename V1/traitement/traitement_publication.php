...
ini_set('display_errors', 1);
    ...
    $pub = $_POST['pub'];
    //echo $pub;
    if(isset($_POST['pub']) && !isset($_FILES['photopub'])){
        to_post($pub, $_SESSION['email']);
    }
    else if (isset($_FILES['photopub'])) {
        $uploadDir = __DIR__ . '/../assets/photo_publication/';
        $maxSize = 2 * 1024 * 1024; // 2 Mo
        $allowedMimeTypes = ['image/jpeg', 'image/png', 'application/pdf'];

        $file = $_FILES['photopub'];
        ...
        to_post_with_files($pub, $filenames, $_SESSION['email']);
    }
